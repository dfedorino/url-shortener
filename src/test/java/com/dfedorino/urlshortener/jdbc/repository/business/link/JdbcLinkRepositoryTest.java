...
class JdbcLinkRepositoryTest extends AbstractJdbcRepositoryTest {
  ...
  void findByUserIdAndStatusId() {
    ...
    tx(() -> {
      assertThat(linkRepository.findByUserIdAndStatusId(user1.getId(), LinkStatus.ACTIVE.getId()))
          .contains(link1);
      assertThat(linkRepository.findByUserIdAndStatusId(user1.getId(), LinkStatus.INVALID.getId()))
          .isEmpty();
      assertThat(linkRepository.findByUserIdAndStatusId(user1.getId(), LinkStatus.DELETED.getId()))
          .isEmpty();

      assertThat(linkRepository.findByUserIdAndStatusId(user2.getId(), LinkStatus.ACTIVE.getId()))
          .contains(link2);
      assertThat(linkRepository.findByUserIdAndStatusId(user2.getId(), LinkStatus.INVALID.getId()))
          .isEmpty();
      assertThat(linkRepository.findByUserIdAndStatusId(user2.getId(), LinkStatus.DELETED.getId()))
          .isEmpty();
    });
  }
  ...
  void updateOriginalUrlByUserIdAndCode() {
    ...
    tx(() -> {
      ...
      assertThat(linkRepository.findByUserIdAndCode(user.getId(), randomCode))
          .get()
          .satisfies(link -> assertThat(link.getOriginalUrl()).isEqualTo(updatedOriginalUrl));
    });
  }
  ...
  void updateRedirectLimitByUserIdAndCode() {
    ...
    assertThat(tx(() -> linkRepository.updateRedirectLimitByUserIdAndCode(user.getId(), randomCode,
        newRedirectLimit)))
        .isTrue();
  ...
  void updateStatusIdByUserIdAndCode() {
    ...
    tx(() -> {
      ...
      assertThat(linkRepository.findByUserIdAndCode(user.getId(), randomCode))
          .get()
          .satisfies(link -> assertThat(link.getStatusId()).isEqualTo(LinkStatus.INVALID.getId()));
    });
  }
}